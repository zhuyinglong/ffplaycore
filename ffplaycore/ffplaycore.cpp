...
static ShowMode sshow_mode = SHOW_MODE_NONE;
...
static void video_image_display(VideoState *is)
{
    ...
    SDL_RenderCopyEx(renderer, is->vid_texture, NULL, &rect, 0, NULL, vp->flip_v ? (SDL_RendererFlip)SDL_FLIP_VERTICAL : (SDL_RendererFlip)0);
    if (sp) {
...
static void video_audio_display(VideoState *s)
{
    ...
    if (!s->paused) {
        ...
        i_start= x = compute_mod(static_cast<int>(s->sample_array_index - delay * channels), SAMPLE_ARRAY_SIZE);
        ...
        if (!s->rdft || !s->rdft_data){
            ...
            for (ch = 0; ch < nb_display_channels; ch++) {
                ...
                for (x = 0; x < 2 * nb_freq; x++) {
                    ...
                    data[ch][x] = static_cast<float>(s->sample_array[i] * (1.0 - w * w));
                    i += channels;
                    if (i >= SAMPLE_ARRAY_SIZE)
                        i -= SAMPLE_ARRAY_SIZE;
                }
...
static void stream_component_close(VideoState *is, uint32_t stream_index)
{
    ...
    do {
        ...
        if (got_frame) {
            ...
            while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0) {
				...
				AVRational tbb = { frame->nb_samples, frame->sample_rate };
                af->duration = av_q2d(tbb);
...
#endif
			AVRational tbb = { frame_rate.den, frame_rate.num };
            duration = (frame_rate.num && frame_rate.den ? av_q2d(tbb) : 0);
...
static int read_thread(void *arg)
{
    ...
    int err, ret;
	unsigned int i, orig_nb_streams;
            ...
            do_seek:
                    if (seek_by_bytes) {
                        ...
                        if (cur_stream->ic->bit_rate)
                            ...
                            incr *= 180000.0;
                        pos = pos + (uint64_t)(incr);
                        stream_seek(cur_stream, pos, (uint64_t)incr, 1);
                    } else {
                        pos = (int64_t)get_master_clock(cur_stream);
                        if (isnan((double)pos))
                            pos = (int64_t)cur_stream->seek_pos / AV_TIME_BASE;
						pos = pos + (uint64_t)(incr);
                        if (cur_stream->ic->start_time != AV_NOPTS_VALUE && pos < cur_stream->ic->start_time / (double)AV_TIME_BASE)
                            pos = (int64_t)(cur_stream->ic->start_time / (double)AV_TIME_BASE);
        ...
        case SDL_MOUSEMOTION:
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN) {
                ...
                if (seek_by_bytes || cur_stream->ic->duration <= 0) {
                    ...
                    ts = (int64_t)frac * cur_stream->ic->duration;
...
{
    sshow_mode =!strcmp(arg, "video") ? SHOW_MODE_VIDEO :
...
#include "cmdutils_common_opts.h"
...
#if CONFIG_AVFILTER
    { "vf", OPT_EXPERT | HAS_ARG, { (void*) = opt_add_vfilter }, "set video filters", "filter_graph" },
    ...
    { "codec", HAS_ARG, { (void*)opt_codec}, "force decoder", "decoder_name" },
